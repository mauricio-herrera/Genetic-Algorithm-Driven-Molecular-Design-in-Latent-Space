...
def compute_kernel(lls, lsf, x, z):
    ...
    r2 = T.outer(T.sum(x * x / lsre, 1), T.ones_like(z[:, 0])) - 2.0 * T.dot(x / lsre, T.transpose(z)) + T.dot(1.0 / lsre, T.transpose(z)**2)
    k = sf * T.exp(-0.5 * r2)
    return k
...
def compute_psi1(lls, lsf, xmean, xvar, z):
    ...
    r2_psi1 = T.outer(T.sum(xmean * xmean / lspxvar, 1), T.ones_like(z[:, 0])) - 2.0 * T.dot(xmean / lspxvar, T.transpose(z)) + T.dot(1.0 / lspxvar, T.transpose(z)**2)
    psi1 = sf * T.outer(constterm2, T.ones_like(z[:, 0])) * T.exp(-0.5 * r2_psi1)
    return psi1
...
def compute_psi1_numpy(lls, lsf, xmean, xvar, z):
    ...
    r2_psi1 = np.outer(np.sum(xmean * xmean / lspxvar, 1), np.ones(z.shape[0])) - 2 * np.dot(xmean / lspxvar, z.T) + np.dot(1.0 / lspxvar, z.T**2)
...
def compute_psi2(lls, lsf, z, input_means, input_vars):
    ...
    r2b = T.sum(scaled_z_minus_m**2, 2)[:, None, :] + T.sum(scaled_z_minus_m**2, 2)[:, :, None] - 2 * T.batched_dot(scaled_z_minus_m, T.transpose(scaled_z_minus_m, [0, 2, 1]))
    term_2 = T.exp(-r2b)
...
def compute_psi2_numpy(lls, lsf, z, input_means, input_vars):
    ...
    scaled_z = z[None, :, :] / scale[:, None, :]
    scaled_z_minus_m = scaled_z
    r2b = np.sum(scaled_z_minus_m**2, 2)[:, None, :] + np.sum(scaled_z_minus_m**2, 2)[:, :, None] - 2 * np.einsum('ijk,ikl->ijl', scaled_z_minus_m, np.transpose(scaled_z_minus_m, [0, 2, 1]))
    term_2 = np.exp(-r2b)
